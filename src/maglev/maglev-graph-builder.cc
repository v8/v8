...
ReduceResult MaglevGraphBuilder::TryReduceRegExpPrototypeTest(
    compiler::JSFunctionRef target, CallArguments& args) {
  // Need the first argument {search_string}.
  if (args.count() == 0) {
    return ReduceResult::Fail();
  }
  ...
  ValueNode* receiver = GetTaggedOrUndefined(args.receiver());
  if (compiler::OptionalHeapObjectRef maybe_receiver_constant =
          TryGetConstant(receiver)) {
    maybe_receiver_map = maybe_receiver_constant.value().map(broker());
  } else {
    auto node_info = known_node_aspects().TryGetInfoFor(receiver);
    // If the map set is not found, then we don't know anything about the map of
    // the receiver, so bail.
    if (!node_info || !node_info->possible_maps_are_known()) {
      if (v8_flags.trace_maglev_graph_building) {
        std::cout << "  ! Failed to reduce Regexp.prototype.test - unknown "
                     "receiver map"
                  << std::endl;
      }
      return ReduceResult::Fail();
    ...
    maybe_receiver_map = possible_maps.at(0);
  }
  ...
  // Only the initial JSRegExp map is valid here.
  compiler::MapRef regexp_initial_map =
      broker()->target_native_context().regexp_function(broker()).initial_map(
          broker());
  if (!receiver_map.equals(regexp_initial_map)) {
    return ReduceResult::Fail();
  }
  ...
  access_infos.push_back(broker()->GetPropertyAccessInfo(
      receiver_map, broker()->exec_string(), compiler::AccessMode::kLoad));
  compiler::PropertyAccessInfo ai_exec =
      access_info_factory.FinalizePropertyAccessInfosAsOne(
          access_infos, compiler::AccessMode::kLoad);
  if (ai_exec.IsInvalid() || !ai_exec.IsFastDataConstant()) {
    return ReduceResult::Fail();
  }

  // Do not reduce if the exec method is not on the prototype chain.
  compiler::OptionalJSObjectRef holder = ai_exec.holder();
  if (!holder.has_value()) return ReduceResult::Fail();

  // Bail out if the exec method is not the original one.
  compiler::OptionalObjectRef constant = holder->GetOwnFastDataProperty(
      broker(), ai_exec.field_representation(), ai_exec.field_index(),
      broker()->dependencies());
  if (!constant.has_value() ||
      !constant->equals(
          broker()->target_native_context().regexp_exec_function(broker()))) {
    return ReduceResult::Fail();
  }
  ...
  BuildCheckString(search_string);

  ValueNode* lastIndex =
      AddNewNode<LoadTaggedField>({receiver}, JSRegExp::kLastIndexOffset);
  BuildCheckSmi(lastIndex);
  // Check positive.
  ValueNode* zero = GetInt32Constant(0);
  // TODO(victorgomes): A Smi bitcast would be enough here. No need to untag.
  ValueNode* lastIndex_int32 = GetInt32(lastIndex);
  AddNewNode<CheckInt32Condition>({lastIndex_int32, zero},
