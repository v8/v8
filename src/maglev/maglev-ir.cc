...
void Typeof::SetValueLocationConstraints() {
  UseRegister(value_input());
  DefineAsRegister(this);
  set_temporaries_needed(2);
}
void Typeof::GenerateCode(MaglevAssembler* masm, const ProcessingState& state) {
  Register value = ToRegister(value_input());
  Register result_reg = ToRegister(result());
  MaglevAssembler::ScratchRegisterScope temps(masm);
  Register scratch = temps.Acquire();

  Label done, return_number, return_function, return_undefined, return_object,
      return_string, return_bigint, return_symbol, is_oddball;

  __ JumpIfSmi(value, &return_number, Label::Distance::kNear);
  __ CompareMapWithRoot(value, RootIndex::kHeapNumberMap, scratch);
  __ JumpIf(kEqual, &return_number, Label::Distance::kNear);

  Register map = temps.Acquire();
  __ LoadMap(map, value);

  Register instance_type = scratch;
  __ LoadUnsignedField(instance_type,
                       FieldMemOperand(map, Map::kInstanceTypeOffset), 2);
  __ CompareInt32AndJumpIf(instance_type, ODDBALL_TYPE, kEqual, &is_oddball);

  Register bitfield = map;
  __ LoadUnsignedField(bitfield, FieldMemOperand(map, Map::kBitFieldOffset), 4);
  __ TestInt32AndJumpIfAnySet(bitfield, Map::Bits1::IsCallableBit::kMask,
                              &return_function, Label::Distance::kNear);
  __ TestInt32AndJumpIfAnySet(bitfield, Map::Bits1::IsUndetectableBit::kMask,
                              &return_undefined, Label::Distance::kNear);

  static_assert(LAST_JS_RECEIVER_TYPE == LAST_TYPE);
  __ CompareInt32AndJumpIf(instance_type, FIRST_JS_RECEIVER_TYPE,
                           kGreaterThanEqual, &return_object,
                           Label::Distance::kNear);
  static_assert(INTERNALIZED_STRING_TYPE == FIRST_TYPE);
  __ CompareInt32AndJumpIf(instance_type, FIRST_NONSTRING_TYPE, kLessThan,
                           &return_string, Label::Distance::kNear);
  __ CompareInt32AndJumpIf(instance_type, BIGINT_TYPE, kEqual, &return_bigint,
                           Label::Distance::kNear);
  if (v8_flags.debug_code) {
    __ CompareInt32(instance_type, SYMBOL_TYPE);
    __ Assert(kEqual, AbortReason::kUnexpectedValue);
  }
  // Fallthrough to return_symbol.

#define LOAD_STRING(name)                               \
  __ bind(&return_##name);                              \
  __ LoadRoot(result_reg, RootIndex::k##name##_string); \
  __ Jump(&done, Label::Distance::kNear);
  LOAD_STRING(symbol);
  LOAD_STRING(number);
  LOAD_STRING(function);
  LOAD_STRING(undefined);
  LOAD_STRING(object);
  LOAD_STRING(string);
  LOAD_STRING(bigint);
#undef LOAD_STRING
  ...
  __ bind(&is_oddball);
  __ LoadTaggedField(result_reg, value, Oddball::kTypeOfOffset);
  // Fallthrough to done.
