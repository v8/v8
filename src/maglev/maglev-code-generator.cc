...
class MaglevFrameTranslationBuilder {
 ...
 private:
  ...
  void BuildFloat64(Float64 number) {
    if (number.is_hole_nan()) {
      translation_array_builder_->StoreLiteral(
          GetDeoptLiteral(ReadOnlyRoots(local_isolate_).the_hole_value()));
    } else {
      Handle<Object> value =
          local_isolate_->factory()->NewNumber<AllocationType::kOld>(
              number.get_scalar());
  ...
  void BuildFieldValue(const FastField value) {
    switch (value.type) {
      ...
      case FastField::kMutableDouble:
        BuildFloat64(value.mutable_double_value);
        break;
    ...
    switch (array.type) {
      ...
      case FastFixedArray::kDouble: {
        ...
        translation_array_builder_->StoreLiteral(
            GetDeoptLiteral(Smi::FromInt(array.length)));
        for (int i = 0; i < array.length; i++) {
          BuildFloat64(array.double_values[i]);
        }
        break;
      }
    }
