...
template <>
Handle<Name> KeyToName<SeededNumberDictionary>(Isolate* isolate,
                                               Handle<Object> key) {
  DCHECK(key->IsSmi());
...
bool AddDescriptorsByTemplate(
  ...
  Handle<SeededNumberDictionary> elements_dictionary =
  ...
  if (elements_dictionary->NumberOfElements() > 0) {
    if (!SubstituteValues<SeededNumberDictionary>(isolate, elements_dictionary,
                                                  receiver, args)) {
      return false;
    }
...
bool InitClassPrototype(Isolate* isolate,
  ...
  Handle<SeededNumberDictionary> elements_dictionary_template(
      SeededNumberDictionary::cast(
          class_boilerplate->instance_elements_template()),
      isolate);
...
bool InitClassConstructor(Isolate* isolate,
  ...
  Handle<SeededNumberDictionary> elements_dictionary_template(
      SeededNumberDictionary::cast(
          class_boilerplate->static_elements_template()),
      isolate);
  Handle<FixedArray> computed_properties(
