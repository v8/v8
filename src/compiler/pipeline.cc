...
PipelineCompilationJob::Status CodeAssemblerCompilationJob::PrepareJobImpl(
  ...
  if (v8_flags.turboshaft_csa) {
    turboshaft_data_.emplace(
        pipeline_->data.zone_stats(), turboshaft::TurboshaftPipelineKind::kCSA,
        isolate, pipeline_->data.info(),
        pipeline_->data.start_source_position(), assembler_options_);
  } else {
  ...
  if (v8_flags.turbo_profiling || v8_flags.dump_builtins_hashes_to_file ||
    ...
    initial_graph_hash_ = HashGraphForPGO(pipeline_->data.graph());
    if (v8_flags.dump_builtins_hashes_to_file) {
      std::ofstream out(v8_flags.dump_builtins_hashes_to_file,
                        std::ios_base::app);
      out << "Builtin: " << Builtins::name(code_assembler_state_.builtin_)
          << ", hash: 0x" << std::hex << initial_graph_hash_ << std::endl;
    }
  }

  if (profile_data_ != nullptr &&
      profile_data_->hash() != initial_graph_hash_) {
    if (v8_flags.reorder_builtins) {
      BuiltinsCallGraph::Get()->set_all_hash_matched(false);
    }
    if (v8_flags.abort_on_bad_builtin_profile_data ||
...
void CodeAssemblerCompilationJob::ExecuteTurbofanJob(RuntimeCallStats* stats) {
  ...
  second_pipeline_->pipeline.SelectInstructionsAndAssemble(call_descriptor);
  ...
  if (jump_opt_ && jump_opt_->is_optimizable()) {
    ...
    pipeline_->pipeline.SelectInstructionsAndAssemble(call_descriptor);
  }
}
...
Pipeline::NewCSLinkageCodeStubBuiltinCompilationJob(
  ...
  return std::make_unique<CodeAssemblerCompilationJob>(
      ...
      CodeKind::BUILTIN, name, profile_data);
}
...
Pipeline::NewBytecodeHandlerCompilationJob(
  ...
  return std::make_unique<CodeAssemblerCompilationJob>(
      ...
      CodeKind::BYTECODE_HANDLER, name, profile_data);
}
...
MaybeHandle<Code> Pipeline::GenerateCodeForCodeStub(
  ...
  if (v8_flags.turboshaft_csa) {
    ...
    CHECK(turboshaft_pipeline.GenerateCode(&linkage, data.osr_helper_ptr(),
                                           jump_optimization_info, profile_data,
                                           initial_graph_hash));
