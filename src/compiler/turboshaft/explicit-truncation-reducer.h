...
class ExplicitTruncationReducer
 ...
 public:
  ...
  OpIndex ReduceOperation(Ts... args) {
    ...
    Op* operation = new (temp_operation_storage_.data()) Op(args...);
 ...
 private:
  ZoneVector<MaybeRegisterRepresentation> inputs_rep_storage_{
      Asm().phase_zone()};
  // TODO: provide some proper max_operation_size.
  static constexpr size_t max_operation_size =
      std::numeric_limits<uint16_t>::max() * 4 + 512;
  ZoneVector<std::byte> temp_operation_storage_{max_operation_size,
                                                Asm().phase_zone()};
};
