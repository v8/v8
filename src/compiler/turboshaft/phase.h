...
struct ComponentWithZone {
  ...
  explicit ComponentWithZone(Zone* existing_zone) : zone(existing_zone) {}
...
class V8_EXPORT_PRIVATE PipelineData {
 ...
 public:
  ...
  void InitializeGraphComponent(Zone* existing_zone, Graph* existing_graph,
                                SourcePositionTable* source_positions,
                                NodeOriginTable* node_origins) {
    DCHECK(!graph_component_.has_value());
    graph_component_.emplace(existing_zone);
    graph_component_->graph = GraphComponent::Pointer<Graph>(existing_graph);
    graph_component_->source_positions =
        GraphComponent::Pointer<SourcePositionTable>(source_positions);
    graph_component_->node_origins =
        GraphComponent::Pointer<NodeOriginTable>(node_origins);
  }
