...
bool IsSupportedSameSimd128OpKind(const Operation& op0, const Operation& op1) {
...
#define CASE(operation)                                \
  ...
  switch (op0.opcode) {
    ...
    CASE(Load)
    CASE(Store)
    CASE(Simd128LaneMemory)  // Force-pack only, ignore the lane kind.
    CASE(Simd128Shuffle)
    case Opcode::kSimd128LoadTransform:
      // Support force-packing, check the transform_kind later.
      return op0.Cast<Simd128LoadTransformOp>().load_kind ==
             op1.Cast<Simd128LoadTransformOp>().load_kind;
    // Support force-packing, check the lane kind later.
    ...
    case Opcode::kPhi:
      return true;
    default:
      // Simd128Test and Simd128ExtractLane don't have Simd128 output.
      // Simd128Reduce and Simd128MoveLane are generated by machine optimizer.
      // They don't expect to occur here. Others (GlobalGet, Parameter,
      // DidntThrow, Projection, StructGet, ArrayGet, etc.) are not supported.
      TRACE("Unsupported operation: %s\n", GetSimdOpcodeName(op0).c_str());
      return false;
  }
#undef CASE
}
...
ShufflePackNode* SLPTree::Try256ShuffleMatchLoad8x8U(
  ...
  DCHECK_EQ(op0.kind, Simd128ShuffleOp::Kind::kI8x16);
  DCHECK_EQ(op1.kind, Simd128ShuffleOp::Kind::kI8x16);

  if (op0.left() == op0.right() || op1.left() == op1.right()) {
    TRACE("Shuffle couldn't be swizzle when match load extend 8x8u!\n");
    return nullptr;
  }
...
#undef CANONICALIZE_SHUFFLE
  if (shuffle0_left_idx != shuffle1_left_idx) {
    TRACE("Shuffles don't have the same left node.\n");
    return nullptr;
  }
  ...
  if (!load_transform) {
    TRACE("Shuffle's left input is not Simd128LoadTransformOp.\n");
    return nullptr;
  }
  ...
  if (!shuffle0_const || !shuffle1_const || !shuffle0_const->IsZero() ||
    ...
    TRACE("Shuffle's right input is not zero.\n");
    return nullptr;
  }
    ...
    */
    ...
    for (int i = 0; i < kSimd128Size / 4; ++i) {
      if (shuffle_copy0[i * 4] != i || shuffle_copy1[i * 4] != i + 4) {
        TRACE("Shuffle's index does not match.\n");
        return nullptr;
      }
      ...
      if (shuffle_copy0[i * 4 + 1] < kSimd128Size ||
        ...
        TRACE("Shuffle's index is not selecting from zero input.\n");
        return nullptr;
      }
    }
...
ShufflePackNode* SLPTree::X64TryMatch256Shuffle(const NodeGroup& node_group,
  ...
  TRACE("Cannot match Simd256Shuffle.\n");
  return nullptr;
}
  ...
  for (const Operation& op : base::Reversed(graph_.operations(block))) {
    ...
    if (IsSupportedReduceSeed(op)) {
      ...
      if (left_index != right_index && left_op.opcode == right_op.opcode &&
          IsSupportedSameSimd128OpKind(left_op, right_op)) {
