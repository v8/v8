...
template <typename PoolEntry>
size_t MemoryPool::PoolImpl<PoolEntry>::ReleaseUpTo(Epoch release_epoch) {
...
size_t MemoryPool::LargePagePoolImpl::ReleaseUpTo(Epoch release_epoch) {
...
MemoryPool::ReleaseStats MemoryPool::ReleaseUpTo(Epoch release_epoch) {
  const auto pages_removed = page_pool_.ReleaseUpTo(release_epoch);
  const auto large_pages_removed = large_pool_.ReleaseUpTo(release_epoch);
  const auto zone_reservations_removed = zone_pool_.ReleaseUpTo(release_epoch);
  return {pages_removed, large_pages_removed, zone_reservations_removed};
}
...
class MemoryPool::ReleasePooledChunksTask final : public CancelableTask {
 public:
  ReleasePooledChunksTask(Isolate* isolate, MemoryPool* pool,
                          Epoch release_epoch)
      : CancelableTask(pool->cancellable_task_manager_.get()),
        isolate_(isolate),
        pool_(pool),
 ...
 private:
  void RunInternal() override {
    ...
    // Repost itself to the next heartbeat.
