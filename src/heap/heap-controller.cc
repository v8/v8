...
size_t MemoryController<Trait>::CalculateAllocationLimit(
  ...
  const size_t limit =
      std::max(static_cast<size_t>(current_size * factor),
               current_size + MinimumAllocationLimitGrowingStep(growing_mode)) +
      new_space_capacity;
  const size_t halfway_to_the_max = (current_size + max_size) / 2;
  const size_t limit_or_halfway = std::min(limit, halfway_to_the_max);
  const size_t result =
      static_cast<size_t>(std::max(limit_or_halfway, min_size));
  if (v8_flags.trace_gc_verbose) {
    Isolate::FromHeap(heap)->PrintWithTimestamp(
