...
char* Shell::ReadCharsFromTcpPort(const char* name, int* size_out) {
  ...
  if (sockfd < 0) {
    i::PrintF(stderr, "Failed to create IPv4 socket\n");
    return nullptr;
  }
  ...
  if (connect(sockfd, reinterpret_cast<sockaddr*>(&serv_addr),
              sizeof(serv_addr)) < 0) {
    i::PrintF(stderr, "Failed to connect to localhost:%d\n",
  ...
  while (sent_len < name_len) {
    ...
    if (sent_now < 0) {
      i::PrintF(stderr, "Failed to send %s to localhost:%d\n", name,
                Shell::options.read_from_tcp_port);
      close(sockfd);
      return nullptr;
    }
  ...
  if (received < 4) {
    i::PrintF(stderr, "Failed to receive %s's length from localhost:%d\n", name,
              Shell::options.read_from_tcp_port);
    close(sockfd);
    return nullptr;
  }
  ...
  if (file_length < 0) {
    i::PrintF(stderr, "Received length %d for %s from localhost:%d\n",
  ...
  while (total_received < file_length) {
    ...
    if (received < 0) {
      i::PrintF(stderr, "Failed to receive %s from localhost:%d\n", name,
                Shell::options.read_from_tcp_port);
      close(sockfd);
      delete[] chars;
      return NULL;
