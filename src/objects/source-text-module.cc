...
MaybeHandle<Object> SourceTextModule::InnerModuleEvaluation(
  ...
  if (module->status() == kEvaluated || module->status() == kEvaluating) {
    return handle(Smi::FromInt(*dfs_index), isolate);
  }

  //    b. Otherwise return module.[[EvaluationError]].
  if (module->status() == kErrored) {
    return handle(module->exception(), isolate);
  }
    ...
    if (requested_module->IsSourceTextModule()) {
      ...
      if (required_module->status() == kEvaluating) {
        ...
        if (required_module->status() == kErrored) {
          return handle(required_module->exception(), isolate);
        }
      }
  ...
  // The spec returns the module index, but before async modules v8 returned
  // the value result from calling next on the module's implicit iterator, we
  // compromise and return the value for synchronous execution and otherwise
  // the module index.
  Handle<Object> result = handle(Smi::FromInt(*dfs_index), isolate);
