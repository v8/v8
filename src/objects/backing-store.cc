...
#define TRACE_BS(...) /* PrintF(__VA_ARGS__); */
...
BackingStore::~BackingStore() {
  ...
  if (is_wasm_memory_) {
    TRACE_BS("BSw:free  bs=%p mem=%p (length=%zu, capacity=%zu)\n", this,
             buffer_start_, byte_length_, byte_capacity_);
    if (is_shared_) {
...
bool BackingStore::GrowWasmMemoryInPlace(Isolate* isolate, size_t new_pages) {
  DCHECK(is_wasm_memory_);
  auto new_byte_length = new_pages * wasm::kWasmPageSize;
