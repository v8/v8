...
TARGET_SVE inline uintptr_t fast_search_sve(T* array, uintptr_t array_len,
  ...
  if constexpr (is_uint32) {
    return search_loop_sve(reinterpret_cast<const uint32_t*>(array), array_len,
                           index, search_element);
  } else if constexpr (is_uint64) {
    return search_loop_sve(reinterpret_cast<const uint64_t*>(array), array_len,
                           index, search_element);
  } else if constexpr (is_double) {
    return search_loop_sve(reinterpret_cast<const float64_t*>(array), array_len,
                           index, search_element);
  }
