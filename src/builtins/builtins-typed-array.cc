...
BUILTIN(Uint8ArrayFromBase64) {
  ...
  // bound, but that requires accessing the string content. Since we resize
  // the backing store after decoding anyway (to handle whitespace), the
  // simpler calculation works.
  size_t max_output_length = (input_length * 3 + 3) / 4;
  size_t output_length = max_output_length;

  DirectHandle<JSArrayBuffer> buffer;
  MaybeDirectHandle<JSArrayBuffer> result_buffer =
      isolate->factory()->NewJSArrayBufferAndBackingStore(
          max_output_length, InitializedFlag::kUninitialized);
  ...
  // If the actual size differs from allocated size (e.g., due to whitespace
  // in input), update byte_length on both JSArrayBuffer and BackingStore to
  // maintain consistency. We use shrink_byte_length instead of ResizeInPlace
  // because ResizeInPlace requires page-aligned memory (for SetPermissions),
  // but regular ArrayBuffer allocations are not page-aligned.
  if (output_length < max_output_length) {
    buffer->set_byte_length(output_length);
    buffer->set_max_byte_length(output_length);
    buffer->GetBackingStore()->shrink_byte_length(output_length);
  }
