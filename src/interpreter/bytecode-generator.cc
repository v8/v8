...
void BytecodeGenerator::BuildClassLiteral(ClassLiteral* expr) {
  ...
  RegisterAllocationScope register_scope(this);
  Register class_constructor_reg;
  {
    RegisterList args = register_allocator()->NewGrowableRegisterList();

    Register class_boilerplate_reg =
        register_allocator()->GrowRegisterList(&args);
    class_constructor_reg = register_allocator()->GrowRegisterList(&args);
    Register super_class_reg = register_allocator()->GrowRegisterList(&args);
    DCHECK_EQ(ClassBoilerplate::kFirstDynamicArgumentIndex,
              args.register_count());

    VisitForAccumulatorValueOrTheHole(expr->extends());
    builder()->StoreAccumulatorInRegister(super_class_reg);

    VisitFunctionLiteral(expr->constructor());
    builder()
        ->StoreAccumulatorInRegister(class_constructor_reg)
        .LoadConstantPoolEntry(class_boilerplate_entry)
        .StoreAccumulatorInRegister(class_boilerplate_reg);

    // Create computed names and method values nodes to store into the literal.
    for (int i = 0; i < expr->properties()->length(); i++) {
      ClassLiteral::Property* property = expr->properties()->at(i);
      if (property->is_computed_name()) {
        Register key_reg = register_allocator()->GrowRegisterList(&args);

        BuildLoadPropertyKey(property, key_reg);
        if (property->is_static()) {
          ...
          builder()
              ...
              .CompareOperation(Token::Value::EQ_STRICT, key_reg)
              .JumpIfFalse(ToBooleanMode::kAlreadyBoolean, &done)
              .CallRuntime(Runtime::kThrowStaticPrototypeError)
              .Bind(&done);
        }
      }
      Register value_reg = register_allocator()->GrowRegisterList(&args);
      VisitForRegisterValue(property->value(), value_reg);
    }
  ...
  if (expr->static_fields_initializer() != nullptr) {
    ...
    builder()
        ->MoveRegister(class_constructor_reg, args[0])
        .CallProperty(initializer, args,
                      feedback_index(feedback_spec()->AddCallICSlot()))
        .LoadAccumulatorWithRegister(class_constructor_reg);
  }
}
