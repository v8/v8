  ...
  for (StackFrameIterator iter(this);; iter.Advance(), visited_frames++) {
    ...
    if (v8_flags.experimental_wasm_stack_switching &&
          ...
          WasmSuspenderObject::cast(root(RootIndex::kActiveSuspender));
      {
        ...
        HandleScope scope(this);
        JSPromise::Reject(handle(suspender.promise(), this),
                          handle(exception, this));
      }
      // Use the return address as the handler. The wrapper will simply return
      // the rejected promise.
      return FoundHandler(Context(), iter.frame()->pc(), 0, kNullAddress,
                          iter.frame()->sp(), iter.frame()->fp(),
                          visited_frames);
    }
#endif
    // Handler must exist.
