...
uint8_t* WasmTrustedInstanceData::memory_base(uint32_t memory_index) const {
  ...
  SBXCHECK_EQ(bases_and_sizes_length % kMemoryBasesAndSizesEntriesPerMemory,
              0u);
  SBXCHECK_LT(memory_index,
              bases_and_sizes_length / kMemoryBasesAndSizesEntriesPerMemory);
  DCHECK_EQ(memory0_start(),
            reinterpret_cast<uint8_t*>(
                memory_bases_and_sizes()->get(kMemoryBasesAndSizesBaseOffset)));
  return reinterpret_cast<uint8_t*>(memory_bases_and_sizes()->get(
      kMemoryBasesAndSizesEntriesPerMemory * memory_index +
...
size_t WasmTrustedInstanceData::memory_size(uint32_t memory_index) const {
  ...
  SBXCHECK_LT(memory_index,
              bases_and_sizes_length / kMemoryBasesAndSizesEntriesPerMemory);
  DCHECK_EQ(memory0_size(),
            memory_bases_and_sizes()->get(kMemoryBasesAndSizesSizeOffset));
  return memory_bases_and_sizes()->get(kMemoryBasesAndSizesEntriesPerMemory *
                                           memory_index +
