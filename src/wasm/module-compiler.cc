...
Address CompileLazy(Isolate* isolate) {
  ...
  if (!lazy_stub->IsAnonymous()) {
    ...
    target_instance =
        handle(lazy_stub->native_module()->compiled_module()->owning_instance(),
               isolate);
  ...
  if (it.frame()->is_js_to_wasm()) {
    DCHECK(!target_instance.is_null());
    ...
    if (target_instance.is_null()) {
      // This is a direct call within the same instance, so the target
      // instance is the same as the calling instance.
      target_instance = handle(wasm_caller_code->native_module()
                                   ->compiled_module()
                                   ->owning_instance(),
                               isolate);
    }
