...
FastCloneObjectMode GetCloneModeForMap(Handle<Map> map, int flags,
  ...
  if (!map->IsJSObjectMap()) {
    ...
    return map->IsNullOrUndefinedMap() || map->IsBooleanMap() ||
                   map->IsHeapNumberMap()
  ...
  // The clone must always start from an object literal map and it must be an
  // instance of the object function. Therefore only for these source maps we
  // can use the source as the target map.
