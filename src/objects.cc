...
void SharedFunctionInfo::AddToOptimizedCodeMap(
    ...
    MaybeHandle<Code> code, BailoutId osr_ast_id) {
  ...
  if (!osr_ast_id.IsNone()) {
    Context::AddToOptimizedCodeMap(native_context, shared,
                                   code.ToHandleChecked(), osr_ast_id);
    return;
  }
...
void JSFunction::EnsureLiterals(Handle<JSFunction> function) {
  ...
  if (function->needs_literals_array()) {
  ...
  } else if (function->literals()->needs_feedback_vector()) {
    if (FLAG_trace_strong_rooted_literals) {
