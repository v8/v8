...
V8_INLINE V8_CLANG_NO_SANITIZE("alignment") bool SimdMemEqual(const LChar* lhs,
  ...
  constexpr int kCharsPerStride = sizeof(__m128i) / RCharSize;
  for (size_t i = count % kCharsPerStride; i < count; i += kCharsPerStride) {
...
V8_INLINE V8_CLANG_NO_SANITIZE("alignment") bool SimdMemEqual(const LChar* lhs,
  ...
  constexpr int kCharsPerStride = sizeof(vec_type) / RCharSize;
  for (size_t i = count % kCharsPerStride; i < count; i += kCharsPerStride) {
