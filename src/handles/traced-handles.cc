  ...
  class Job : public v8::JobTask {
    class SliceView final {
     public:
      ...
      auto begin() const {
        return std::next(young_handles_.begin(),
                         slice_index_ * kHandlesPerWorker);
      }
      ...
      do {
        ...
        for (auto* handle : SliceView(derived_.young_handles_, *index)) {
          if constexpr (IsMainThread) {
            ...
            derived_.ProcessNodeFromConcurrentThread(handle);
          }
...
class TracedHandlesWeaknessProcessor
 ...
 public:
  ...
  void ProcessNodeFromConcurrentThread(TracedNode* node) {
...
class TracedHandlesClearingProcessor final
 ...
 public:
  ...
  void ProcessNodeFromMainThread(TracedNode* node) {
    ProcessNodeImpl</*IsMainThread=*/true>(node);
  }

  void ProcessNodeFromConcurrentThread(TracedNode* node) {
    ProcessNodeImpl</*IsMainThread=*/false>(node);
  }
 ...
 private:
  template <bool IsMainThread>
  void ProcessNodeImpl(TracedNode* node) {
    if (!node->is_in_use()) return;
    ...
    if (should_reset) {
      ...
      embedder_handler_.ResetRoot(
          *reinterpret_cast<v8::TracedReference<v8::Value>*>(&value));
      if constexpr (!IsMainThread) {
        // We could keep store the to-be-destructed handle in a separate
        // vector. However, it turns out to be quite expensive, even for the
        // concurrent thread, so we instead reiterate all the young nodes in
        // Epilogue().
        DCHECK(TracedHandlesImpl::ShouldDeferNodeFreeingForCurrentThread());
      } else {
        // The node should already be destructed, if needed.
        DCHECK(!TracedHandlesImpl::ShouldDeferNodeFreeingForCurrentThread());
      }
