...
utils.ImportFromExperimental = function Import(f) {
  f.next = imports_from_experimental;
  imports_from_experimental = f;
};


utils.SetFunctionName = function SetFunctionName(f, name, prefix) {
  if (IS_SYMBOL(name)) {
...
utils.InstallConstants = function InstallConstants(object, constants) {
  %CheckIsBootstrapping();
...
utils.InstallFunctions = function InstallFunctions(
...
utils.InstallGetter = function InstallGetter(object, name, getter, attributes) {
  %CheckIsBootstrapping();
  if (typeof attributes == "undefined") {
    attributes = DONT_ENUM;
  }
  utils.SetFunctionName(getter, name, "get");
...
utils.InstallGetterSetter = function InstallGetterSetter(
    object, name, getter, setter) {
  %CheckIsBootstrapping();
  utils.SetFunctionName(getter, name, "get");
  utils.SetFunctionName(setter, name, "set");
  %FunctionRemovePrototype(getter);
...
utils.SetUpLockedPrototype = function SetUpLockedPrototype(
...
utils.PostExperimentals = function() {
  %CheckIsBootstrapping();
