...
CodeDataSource::TraceContext::TracePacketHandle NewTracePacket(
  ...
  packet->set_trace_packet_defaults()->set_timestamp_clock_id(
      BuiltinClock::BUILTIN_CLOCK_BOOTTIME);

  auto* v8_defaults =
      packet->set_trace_packet_defaults()->set_v8_code_defaults();
...
class IsolateRegistry {
 public:
  ...
  void Register(Isolate* isolate, PerfettoLogger* logger) {
    base::MutexGuard lock(&mutex_);
    DCHECK(isolates_.emplace(isolate, logger).second);
  }

  void Unregister(Isolate* isolate) {
    base::MutexGuard lock(&mutex_);
    DCHECK(isolates_.erase(isolate));
  }

  void LogExistingCodeForAllIsolates() {
    base::MutexGuard lock(&mutex_);
    for (auto [isolate, listener] : isolates_) {
 ...
 private:
  base::Mutex mutex_;
  absl::flat_hash_map<Isolate*, PerfettoLogger*> isolates_;
};
...
void PerfettoLogger::RegisterIsolate(Isolate* isolate) {
  auto listener = new PerfettoLogger(isolate);
  isolate->logger()->AddListener(listener);
  IsolateRegistry::GetInstance().Register(isolate, listener);
...
// static
void PerfettoLogger::LogExistingCodeForAllIsolates() {
  IsolateRegistry::GetInstance().LogExistingCodeForAllIsolates();
}
