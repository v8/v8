...
InitializeReservationAndAllocator() {
  ...
  VirtualMemory reservation(
      platform_allocator, kReservationSize, v8::PageAllocator::AllocationHint(),
  ...
  std::unique_ptr<base::BoundedPageAllocator> bpa =
      std::make_unique<base::BoundedPageAllocator>(
          ...
          PageMetadata::kPageSize, base::PageInitializationMode::kRecommitOnly,
...
TEST_F(RedZoneTest, AddWholeReservationAsRedZone) {
  ...
  void* page = allocator->AllocatePages(nullptr, kPageSize, kPageSize,
                                        PageAllocator::Permission::kNoAccess);
  EXPECT_NE(nullptr, page);
  // Free the memory to return into initial state.
  EXPECT_TRUE(allocator->FreePages(page, kPageSize));
  ...
  EXPECT_EQ(nullptr,
            allocator->AllocatePages(nullptr, PageMetadata::kPageSize,
