...
TEST_F(MemoryControllerTest, OldGenerationAllocationLimit) {
  ...
  EXPECT_EQ(static_cast<size_t>(old_gen_size * factor),
            V8HeapTrait::BoundAllocationLimit(
                old_gen_size, old_gen_size * factor, 0u,
                max_old_generation_size, Heap::HeapGrowingMode::kDefault));

  factor = std::min({factor, V8HeapTrait::kConservativeGrowingFactor});
  EXPECT_EQ(static_cast<size_t>(old_gen_size * factor),
            V8HeapTrait::BoundAllocationLimit(
                old_gen_size, old_gen_size * factor, 0u,
                max_old_generation_size, Heap::HeapGrowingMode::kSlow));

  factor = std::min({factor, V8HeapTrait::kConservativeGrowingFactor});
  EXPECT_EQ(static_cast<size_t>(old_gen_size * factor),
            V8HeapTrait::BoundAllocationLimit(
                old_gen_size, old_gen_size * factor, 0u,
                max_old_generation_size, Heap::HeapGrowingMode::kConservative));

  factor = V8HeapTrait::kMinGrowingFactor;
  EXPECT_EQ(static_cast<size_t>(old_gen_size * factor),
            V8HeapTrait::BoundAllocationLimit(
                old_gen_size, old_gen_size * factor, 0u,
                max_old_generation_size, Heap::HeapGrowingMode::kMinimal));
