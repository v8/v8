...
TEST_F(MacroAssemblerTest, jump_tables5) {
...
TEST_F(MacroAssemblerTest, Dlsa) {
  struct TestCaseLsa {
    int64_t rt;
    int64_t rs;
    uint8_t sa;
...
TEST_F(MacroAssemblerTest, cvt_d_l_Trunc_l_ud) {
  FOR_INT64_INPUTS(i, cvt_trunc_int64_test_values) {
    int64_t input = *i;
...
TEST_F(MacroAssemblerTest, Ulwc1) {
  static const int kBufferSize = 300 * KB;
  char memory_buffer[kBufferSize];
  char* buffer_middle = memory_buffer + (kBufferSize / 2);
...
TEST_F(MacroAssemblerTest, Uldc1) {
  static const int kBufferSize = 300 * KB;
  char memory_buffer[kBufferSize];
  char* buffer_middle = memory_buffer + (kBufferSize / 2);
