...
TEST(UtilsTest, CompareCharsEqual) {
  constexpr size_t kMaxLen = 65;
  constexpr uint8_t kMismatchChar = kMaxLen + 1;
  uint8_t one_byte_ref[kMaxLen];
  uint8_t one_byte_test[kMaxLen];
  uint16_t two_byte_ref[kMaxLen];
  uint16_t two_byte_test[kMaxLen];
  ...
  constexpr size_t lengths[] = {0, 1, 2, 3, 4, 5, 8, 9, 16, 17, 32, 33, 42, 64};

  for (size_t len : lengths) {
    ASSERT_LE(len, kMaxLen);
    SCOPED_TRACE(len);
    EXPECT_TRUE(CompareCharsEqual(one_byte_ref, one_byte_test, len));
    EXPECT_TRUE(CompareCharsEqual(two_byte_ref, two_byte_test, len));
    EXPECT_TRUE(CompareCharsEqual(one_byte_ref, two_byte_ref, len));

    if (len == 0) continue;

    constexpr size_t modification_offsets[] = {0, 1,  2,  3,  4,  7,
                                               8, 15, 16, 17, 31, 32};
    for (size_t offset : modification_offsets) {
      if (offset >= len) continue;
      SCOPED_TRACE(offset);

      // Introduce a mismatch.
      one_byte_test[offset] = kMismatchChar;
      two_byte_test[offset] = kMismatchChar;

      EXPECT_FALSE(CompareCharsEqual(one_byte_ref, one_byte_test, len));
      EXPECT_FALSE(CompareCharsEqual(two_byte_ref, two_byte_test, len));
      EXPECT_FALSE(CompareCharsEqual(one_byte_ref, two_byte_test, len));

      // Restore for next iteration.
      one_byte_test[offset] = offset;
      two_byte_test[offset] = offset;
    }

    // Test mismatch at the last character.
    size_t last_offset = len - 1;
    one_byte_test[last_offset] = kMismatchChar;
    two_byte_test[last_offset] = kMismatchChar;
    EXPECT_FALSE(CompareCharsEqual(one_byte_ref, one_byte_test, len));
    EXPECT_FALSE(CompareCharsEqual(two_byte_ref, two_byte_test, len));
    EXPECT_FALSE(CompareCharsEqual(one_byte_ref, two_byte_test, len));
    one_byte_test[last_offset] = last_offset;
    two_byte_test[last_offset] = last_offset;
  }
}
