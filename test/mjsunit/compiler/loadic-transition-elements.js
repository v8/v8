...
%OptimizeFunctionOnNextCall(accessArray);

// A function with a pristine AllocationSite.
function allocateArray2() {
  return [4, 5, 6];
}
%PrepareFunctionForOptimization(allocateArray2);

let smiArray2 = allocateArray2();
assertTrue(%HasSmiElements(smiArray2));

accessArray(smiArray2);

if (hasMementos) {
  // Loading the element from smiArray2 updated the AllocationSite in
  // allocateArray2.
  let newArray = allocateArray2();
  assertTrue(%HasDoubleElements(newArray));
}
