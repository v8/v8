...
// Forward declaration.
Handle<InstructionStream> BuildTeardownFunction(
...
class TestEnvironment : public HandleAndZoneScope {
 public:
  ...
  Handle<FixedArray> Run(Handle<InstructionStream> test,
...
#endif
      Handle<InstructionStream> setup = BuildSetupFunction(
...
// Wrapper around the CodeGenerator. InstructionStream generated by this can
...
class CodeGeneratorTester {
 public:
  ...
  Handle<InstructionStream> Finalize() {
  ...
  Handle<InstructionStream> FinalizeForExecuting() {
...
TEST(FuzzAssembleMove) {
  ...
  for (int extra_space : {0, kExtraSpace}) {
    ...
    Handle<InstructionStream> test = c.FinalizeForExecuting();
...
TEST(AssembleTailCallGap) {
...

  {
    ...
    Handle<InstructionStream> code = c.Finalize();
    if (v8_flags.print_code) {
      code->Print();
    }
  }
