...
class ConditionalSelectGen {
 public:
  ...
  ResultType expected(CondType lhs, CondType rhs, ResultType tval,
    ...
    ResultType res =
        Is32() ? cmpw().Int32Compare(static_cast<int32_t>(bin_node_res), 0)
                     ? tval
                     : fval
        : cmpw().Int64Compare(static_cast<int64_t>(bin_node_res), 0) ? tval
                                                                     : fval;
...
class Int32ConditionalSelectGen
    : public ConditionalSelectGen<int32_t, ResultType> {
 public:
  using ConditionalSelectGen<int32_t, ResultType>::ConditionalSelectGen;
  ...
  Node* AddBinopUse() override {
    ...
    switch (select->opcode()) {
      case IrOpcode::kFloat32Select:
        bin_node_use = m.RoundInt32ToFloat32(bin_node);
        add_op = m.machine()->Float32Add();
        break;
      case IrOpcode::kFloat64Select:
        bin_node_use = m.ChangeInt32ToFloat64(bin_node);
      ...
      case IrOpcode::kWord32Select:
        ...
        break;
      case IrOpcode::kWord64Select:
        bin_node_use = m.ChangeInt32ToInt64(bin_node);
...
template <typename ResultType>
class Int64ConditionalSelectGen
    : public ConditionalSelectGen<int64_t, ResultType> {
 public:
  using ConditionalSelectGen<int64_t, ResultType>::ConditionalSelectGen;
  ...
  Node* AddBinopUse() override {
    ...
    switch (select->opcode()) {
      case IrOpcode::kFloat32Select:
        bin_node_use = m.RoundInt64ToFloat32(bin_node);
        add_op = m.machine()->Float32Add();
        break;
      case IrOpcode::kFloat64Select:
        bin_node_use = m.ChangeInt64ToFloat64(bin_node);
        add_op = m.machine()->Float64Add();
        break;
      case IrOpcode::kWord32Select:
    ...
    for (auto cmp : int32_cmp_opcodes) {
      for (auto bin : int32_bin_opcodes) {
        BufferedRawMachineAssemblerTester<int32_t> m(
            ...
            MachineType::Int32());
        Int32ConditionalSelectGen<int32_t> gen(m, config, cmp, bin);
        ...
        FOR_INT32_INPUTS(a) {
          FOR_INT32_INPUTS(b) {
            int32_t expected = gen.expected(a, b, 2, 1);
            int32_t actual = m.Call(a, b, 2, 1);
    ...
    for (auto cmp : int32_cmp_opcodes) {
      for (auto bin : int32_bin_opcodes) {
        BufferedRawMachineAssemblerTester<int64_t> m(
            MachineType::Int32(), MachineType::Int32(), MachineType::Int64(),
            MachineType::Int64());
        Int32ConditionalSelectGen<int64_t> gen(m, config, cmp, bin);
        Node* lhs = m.Parameter(0);
        Node* rhs = m.Parameter(1);
        Node* tval = m.Parameter(2);
        Node* fval = m.Parameter(3);
        ...
        FOR_INT32_INPUTS(a) {
          FOR_INT32_INPUTS(b) {
            int64_t c = 2;
            int64_t d = 1;
            int64_t expected = gen.expected(a, b, c, d);
            int64_t actual = m.Call(a, b, c, d);
            CHECK_EQ(expected, actual);
          }
        }
      }
    ...
    for (auto cmp : int32_cmp_opcodes) {
      for (auto bin : int32_bin_opcodes) {
        BufferedRawMachineAssemblerTester<float> m(
            MachineType::Int32(), MachineType::Int32(), MachineType::Float32(),
            MachineType::Float32());
        Int32ConditionalSelectGen<float> gen(m, config, cmp, bin);
        Node* lhs = m.Parameter(0);
        Node* rhs = m.Parameter(1);
        Node* tval = m.Parameter(2);
        Node* fval = m.Parameter(3);
    ...
    for (auto cmp : int32_cmp_opcodes) {
      for (auto bin : int32_bin_opcodes) {
        BufferedRawMachineAssemblerTester<double> m(
            MachineType::Int32(), MachineType::Int32(), MachineType::Float64(),
            MachineType::Float64());
        Int32ConditionalSelectGen<double> gen(m, config, cmp, bin);
        Node* lhs = m.Parameter(0);
        Node* rhs = m.Parameter(1);
        Node* tval = m.Parameter(2);
        Node* fval = m.Parameter(3);
    ...
    for (auto cmp : int64_cmp_opcodes) {
      for (auto bin : int64_bin_opcodes) {
        BufferedRawMachineAssemblerTester<int32_t> m(
            ...
            MachineType::Int32());
        Int64ConditionalSelectGen<int32_t> gen(m, config, cmp, bin);
        Node* lhs = m.Parameter(0);
        Node* rhs = m.Parameter(1);
        Node* tval = m.Parameter(2);
        Node* fval = m.Parameter(3);
    ...
    for (auto cmp : int64_cmp_opcodes) {
      for (auto bin : int64_bin_opcodes) {
        BufferedRawMachineAssemblerTester<int64_t> m(
            ...
            MachineType::Int64());
        Int64ConditionalSelectGen<int64_t> gen(m, config, cmp, bin);
        Node* lhs = m.Parameter(0);
        Node* rhs = m.Parameter(1);
        Node* tval = m.Parameter(2);
        Node* fval = m.Parameter(3);
    ...
    for (auto cmp : int64_cmp_opcodes) {
      for (auto bin : int64_bin_opcodes) {
        BufferedRawMachineAssemblerTester<float> m(
            ...
            MachineType::Float32());
        Int64ConditionalSelectGen<float> gen(m, config, cmp, bin);
        Node* lhs = m.Parameter(0);
        Node* rhs = m.Parameter(1);
        Node* tval = m.Parameter(2);
        Node* fval = m.Parameter(3);
...
TEST(Float64SelectCombineInt64CompareZero) {
  ...
  for (auto config : graph_configs) {
    for (auto cmp : int64_cmp_opcodes) {
      for (auto bin : int64_bin_opcodes) {
        BufferedRawMachineAssemblerTester<double> m(
            MachineType::Int64(), MachineType::Int64(), MachineType::Float64(),
            MachineType::Float64());
        Int64ConditionalSelectGen<double> gen(m, config, cmp, bin);
        Node* lhs = m.Parameter(0);
        Node* rhs = m.Parameter(1);
        Node* tval = m.Parameter(2);
        Node* fval = m.Parameter(3);
